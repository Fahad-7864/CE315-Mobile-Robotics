...
public:
    ...
    double laser_landmark3 = 0.7, laser_landmark4 = 0.6, laser_landmark5 = 0.4;
    int stage = 1;
    ...
    Stopper() : Node("Stopper") {
        laserScan_ = this->create_subscription<sensor_msgs::msg::LaserScan>("scan", 10,
                                                                            std::bind(&Stopper::scanCallback, this,
                                                                                      std::placeholders::_1));
...
switch(stage){
    case 1:
        ...
        case 2:
            ...
            break;
      	case 3:
            if (mrightRange > laser_landmark3)
                moveForwardRight(FORWARD_SPEED_MIDDLE,TURN_LEFT_SPEED_MIDDLE);
            else stage = 4;
            break;
       case 4:
            if (rightRange < laser_landmark4)
                moveForward(FORWARD_SPEED_HIGH);
            else stage = 4;
            break;     
      	case 5:
            if (mleftRange < laser_landmark5)
                moveForward(FORWARD_SPEED_MIDDLE);
            else stage = 5;
            break;
...
void Stopper::startMoving() {
    ...
    laserFile.open("/ufs/servh01/users/fh17602/M-Drive/ros_workspace/src/tutorial_pkg/laserData.csv",ios::trunc); //note you should modify hhu to your userna
    laserMapFile.open("/ufs/servh01/users/fh17602/M-Drive/ros_workspace/src/tutorial_pkg/laserMapData.csv",ios::trunc); //note you should modify hhu to your userna
    RCLCPP_INFO(this->get_logger(), "Start moving");
    rclcpp::WallRate loop_rate(10);

    while (rclcpp::ok()) { // keep spinning loop until you press Ctrl+C
